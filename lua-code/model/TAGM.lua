...
function TAGM.model(loader, opt)
  ...
  local top_lstm_variant, top_bilstm_variant, att_fusion
  ...
  if opt.gpuid >= 0 then top_lstm_variant:cuda()  end
  ...
  TAGM.top_c = top_c
  if opt.gpuid >= 0 then
    TAGM.top_c = TAGM.top_c:cuda()
    TAGM.lstm  = TAGM.lstm:cuda()
    TAGM.pre_m = TAGM.pre_m:cuda()
  end
...
function TAGM.backward(x, attention_weight, opt, gradout, loader)
  ...
  local rnn_input = x
  if opt.gpuid >= 0 then
    pre_out = TAGM.pre_out:cuda()
  else
    pre_out = TAGM.pre_out
  end
