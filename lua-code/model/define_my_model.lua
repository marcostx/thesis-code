...
function my_model.define_model(opt, loader, if_evaluate_from_scratch)
  ...
  if opt.multimodal and opt.early == false then
    ...
    Multimodal_Fusion.model(loader, opt)
  end
  ...
  elseif opt.top_c == 'TAGM' then
    if opt.if_attention == 1 then
      if opt.top_bidirection == 0 then
        if opt.multimodal and opt.early == false then
          params_flat, grad_params_flat = model_utils.combine_all_parameters(attention.rnn,attention.birnn, Multimodal_Fusion.att_fusion_model1,
            Multimodal_Fusion.att_fusion_model2,attention.weight_net, top_net.pre_m, top_net.lstm, top_net.top_c)
        else
          params_flat, grad_params_flat = model_utils.combine_all_parameters(attention.rnn, attention.birnn, 
  ...
  if opt.multimodal and opt.early == false then
    print("cloning multimodal fusion net")
    Multimodal_Fusion.clone_model(loader)
  end
  if opt.top_c ~= 'NN' then
    top_net.clone_model(loader, opt)
  end
  model.attention = attention
  model.att_fusion = Multimodal_Fusion
