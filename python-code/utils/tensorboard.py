...
def save_graph(model, device, dataloader, tb_writer):
    with torch.no_grad():
        for batch in dataloader:
            tb_writer.add_graph(
                model, input_to_model=batch['x'].float().to(device))
            break
...
def save_weigths(model, tb_writer, epoch):
    for name, value in model.named_parameters():
        ...
        tb_writer.add_histogram(
            "Weigths/{}".format(name), value, global_step=epoch)


def save_grads(grad_dict, tb_writer, epoch):
    for name, value in grad_dict.items():
        name = "-".join(name.split("."))
        tb_writer.add_histogram("Grads/{}".format(name),
...
def save_confusion_matrix(correct_labels, predict_labels, labels,  tb_writer, epoch, normalize=False, title='Confusion matrix'):
    ...
    cm, type = create_confusion_matrix(
        correct_labels, predict_labels, labels, normalize)
    ...
    for i, j in itertools.product(range(cm.shape[0]), range(cm.shape[1])):
        ax.text(j, i, format(cm[i, j], type), horizontalalignment="center",
                fontsize=8, verticalalignment='center', color="black")
